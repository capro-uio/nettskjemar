...
# Retrieving Metadata for a Form: `nskj_get_meta`

The function `nskj_get_meta` retrieves metadata for a given form using its **form ID**.
The retrieved metadata is returned as an object of class `nskj_meta`.
...
form_meta <- nskj_get_meta(formid)
...
# Saving Metadata to a File: `nskj_write_meta`

The metadata retrieved using `nskj_get_meta` can be saved to a file for safe keeping or further use.
The `nskj_write_meta` function writes the metadata into a JSON file, preserving all its information in a structured format.
...
nskj_write_meta(form_meta, output_path)
```
...
nskj_write_meta(form_meta, "meta_file")
...
The `nskj_write_meta` function supports any additional arguments passed to the [jsonlite::write_json](https://cran.r-project.org/package=jsonlite) function.
For example, you can specify `pretty = TRUE` for a more human-readable format.

```r
# Save the metadata in a pretty-printed JSON file
nskj_write_meta(form_meta, "pretty_meta.json", pretty = TRUE)
```
