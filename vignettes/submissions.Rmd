...
# Getting an Individual Submission Answer: `nskj_get_submission`

The `nskj_get_submission` function fetches a single submission's answers based on a **submission ID**.
...
# Downloading a Submission as a PDF: `nskj_get_submission_pdf`

The `nskj_get_submission_pdf` function downloads a single submission as a PDF file.
...
nskj_get_submission_pdf(submission_id)
```
...
nskj_get_submission_pdf(submission_id, path = custom_path)
```
