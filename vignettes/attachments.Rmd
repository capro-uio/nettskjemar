...
Each submission ID may have associated attachments. Use `nskj_list_submission_attachments()` to retrieve information about the available attachments.
...
To list all attachments across multiple submissions for a form, use `nskj_list_form_attachments()`.


``` r
# Example: List attachments for an entire form
formid <- 123823
nskj_list_form_attachments(formid)
#>    formId submissionId  answerId elementId
#> 16 123823     27685292 158263134   1641713
#> 32 123823     27685302 158263185   3525020
...
Once you have identified the attachments, you can download them using `nskj_get_attachment()`.

```r
# Example: Download a single attachment
attachment_id <- "12345"
nskj_get_attachment(attachment_id, path = "downloaded_file.pdf")
```
...
Use `nskj_get_submission_attachments()` to download and save all attachments associated with a single submission.
...
```r
nskj_get_submission_attachments(submission_id, filenames = "original")
```

You can also specify an output directory:

```r
nskj_get_submission_attachments(submission_id, output_dir = "my_attachments/")
```
...
```r
nskj_get_form_attachments(formid)
```

As with submission attachments, you can specify a filename format and output directory:

```r
nskj_get_form_attachments(formid, filenames = "original", output_dir = "all_attachments/")
```
