...
#' \code{\link{nskj_list_submission_attachments}},
...
#' #attach_dt <- nskj_list_submission_attachments(submission_id)
#' #nskj_save_attachment(path = attach_dt$path, output = attach_dt$standardized)
#' }
nskj_get_attachment <- function(attachment_id, path = NULL) {
  resp <- nskj_req() |>
...
#' \code{\link{nskj_list_submission_attachments}} for idea
#' of the names used.
#'
...
#' \dontrun{
#' form_id <- 1100000
#' nskj_list_form_attachments(form_id)
#'
#' }
nskj_list_form_attachments <- function(form_id) {
...
#'
#' @template form_id
#' @inheritParams nskj_get_submission_attachments
#'
#' @return invisible named logical if the file was saved successfully
#' @export
#'
...
nskj_get_form_attachments <- function(
  ...
  ids <- nskj_list_form_attachments(form_id)
...
#' \dontrun{
#' submission_id <- 22222
#' nskj_list_submission_attachments(submission_id)
#'
#' }
nskj_list_submission_attachments <- function(submission_id) {
...
#' \dontrun{
#' submission_id <- 22222
#' nskj_get_submission_attachments(submission_id)
#' nskj_get_submission_attachments(submission_id, "original")
#'
#' }
nskj_get_submission_attachments <- function(
  submission_id,
  filenames = "standardized",
  output_dir = paste0("./", submission_id)
) {
  ids <- nskj_list_submission_attachments(submission_id)
