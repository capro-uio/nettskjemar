...
#'
#' @template form_id
#' @return list of class nskj_meta_data
#' @export
#' @examples
#' \dontrun{
...
#'
#' }
nskj_get_meta <- function(form_id){
...
meta_raw <- function(content){
  structure(content,
            class = c("nskj_meta", "list"))
}

#' @export
format.nskj_meta <- function(x, ...){
  c(
    sprintf("# Nettskjema raw metadata for form %s", x$form_id),
    "",
    unname(sapply(c(
      "title", "editorsContactEmail",
      "isOpen", "isCodebookValid", 
      "numberOfSubmissions", 
      "modifiedDate"),
...
#'
#' @param meta object of class nskj_meta class
...
#' \dontrun{
#' form_id <- 1100000
#' my_meta <- nskj_get_meta(form_id)
#' nskj_write_meta(my_meta, "my/path/meta_110000.json")
#' }
nskj_write_meta <- function(meta, path, ...){
  UseMethod("nskj_write_meta")
}

#' @export
#' @rdname nskj_write_meta
nskj_write_meta.default <- function(meta, path, ...){
...
#' @export
#' @rdname nskj_write_meta
nskj_write_meta.nskj_meta <- function(meta, path, ...){
  if(!grepl("json$", path)){
