...
#' @noRd
clean_form_submissions <- function(ndata,
                                   cb,
                                   use_codebook,
                                   checkbox_type = c("string", "list", "columns"),
                                   checkbox_delim = ";") {
  checkbox_type <-
    match.arg(checkbox_type, c("string", "list", "columns"))

  dt <- tibble(submission_id = map_chr(ndata, "submissionId"))
  dt <- bind_cols(dt,
                  bind_rows(
                    map(
                      ndata,
  ...
  if (checkbox_type == "string") {
    sub_it <- function(x) {
      ...
      x <-
        lapply(x, function(x)
          paste0(x, collapse = checkbox_delim))
      sapply(x, function(x)
        ifelse(x == "NA", NA, x))
    }
...
#' @importFrom dplyr as_tibble transmute
#' @noRd
extract_submission_answers <- function(ndata,
                                       cb,
                                       use_codebook) {
  type <- sapply(ndata$answers,
                 function(x)
                   "answerOptions" %in% names(x))
  tmp <- map(ndata$answers, "externalAnswerOptionId")
  if(any(is.null(unlist(tmp))))
    use_codebook <- FALSE

  answ <- lapply(seq_along(ndata$answers),
                 function(x) {
                   tmp <- ndata$answers[[x]]
                   opt <- dplyr::case_when(
                     "textAnswer" %in% names(tmp) ~ "textAnswer",
                     all(use_codebook, "externalAnswerOptionId" %in% names(tmp)) ~ "externalAnswerOptionId",
                     TRUE ~ "text"
                   )
                   ans <- map_chr(
                     tmp[["answerOptions"]],
  ...
  if (any(use_codebook & !inherits(cb, "data.frame"))) {
    names(answ) <- map(ndata$answers, "externalQuestionId")
  } else{
    nms <- dplyr::transmute(cb,
                            question_id,
                            col = paste(element_no, question, sep = "."))
    nms <- nms[nms$question_id %in% names(unlist(answ)), "col"]
    nms <- unique(nms$col)
    names(answ) <- gsub(" ", "_", tolower(nms))
  }
  as_tibble(lapply(answ, function(x)
...
#' @noRd
grab_data <- function(incremental,
                      submissionIds,
                      token_name,
                      path,
                      opts,
  ...
  if (incremental | length(submissionIds) > 2000) {
    if (length(submissionIds) > 2000)
      cat(
        "\tNumber of responses to download exceeds 2000, switching to incremental download.\n"
      )

    submissionIds <- file.path("submissions", submissionIds)

    resp <- pblapply(submissionIds,
                     function(x)
                       nettskjema_api(x,
...
get_extra_data <-
  function(questions,
    ...
    data_extra <- select(data_extra,
                         matches(paste0(
                           information, collapse = "|", sep = "$"
                         )))

    for (inf in 1:length(information)) {
      data_extra <- rename_all(data_extra,
                               rn_cols,
